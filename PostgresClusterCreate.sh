...
# for test
#yc postgres cluster create --name $clustername \
#--environment=production \
#--network-name default \
#--resource-preset s2.micro \
#--host zone-id=ru-central1-c,subnet-name=default-ru-central1-c \
#--disk-size 10 \
#--disk-type network-ssd \
#--user name=$username,password=$password \
#--database name=$dbname,owner=$username

# basic cluster for prod
# hover tune at least username and password
yc postgres cluster create --name $clustername \
--environment=production \
--postgresql-version 12-1c \
--network-name default \
--resource-preset s2.medium \
--host zone-id=ru-central1-c,subnet-name=default-ru-central1-c \
--host zone-id=ru-central1-a,subnet-name=default-ru-central1-a \
--host zone-id=ru-central1-b,subnet-name=default-ru-central1-b \
--disk-size 100 \
--disk-type local-ssd \
--user name=$username,password=$password \
